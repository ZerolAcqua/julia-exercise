...
Submission by: **_Zerol Acqua_** (zerolacqua@outlook.com)
"""
...
@bind l_box Slider(0:20, show_value=true, default=5)

# ╔═╡ 302f0842-453f-47bd-a74c-7942d8c96485
l_box

# ╔═╡ a5435a12-aca4-4ca0-bd17-bea731bb180d
colored_line(v)
...
function box_blur(v::AbstractArray, l)
	extends=(i->extend(v,i))
	result=zeros(length(v))
	for i in 1:length(v)
		result[i]=mean(extends.(collect(i-l:i+l)))
	end
	return result
end
...
function convolve(v::AbstractVector, k)
	# k should be an odd
	result = ones(length(v)) # Float64! Don't use Int64!
	l = length(k) ÷ 2
	for i in 1:length(v)
		result[i]=sum([extend(v,i-x)*k[x+l+1] for x=-l:l])
	end
	return result
end
...
function convolve(M::AbstractMatrix, K::AbstractMatrix)
	# Kernel should be a square matrix and have odd width
	l = size(K)[1]÷2
	rows,cols = size(M)
	result = copy(M)
	for i in 1:rows
		for j in 1:cols
			result[i,j]=sum(
				[extend(M,i-x,j-y)*K[1+l+x,1+l+y] for x=-l:l,y=-l:l]
			)
		end
	end
	return result
...
function gaussian_kernel_1D(n; σ = 1)
	vector = (n->gauss(n; σ)).(collect(-n:n))
...
function with_gaussian_blur(image; σ=3, l=5)
	xmatrix = repeat(gaussian_kernel_1D(l,σ=σ),1,l*2+1)
	ymatrix = repeat(transpose(gaussian_kernel_1D(l,σ=σ)),l*2+1)
	gaussian_kernel = xmatrix.*ymatrix
...
function with_sobel_edge_detect(image)
	gx=[1 0 -1;
		...
		1 0 -1]
	gy=transpose(gx)
	return 	sqrt.(convolve(image,gx).^2+convolve(image,gy).^2)
end
